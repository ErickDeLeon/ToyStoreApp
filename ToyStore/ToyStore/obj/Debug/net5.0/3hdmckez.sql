...
VALUES (51, 'Jenga', 'jenga Hasbro a los must have de diversión que no podrán faltar en tu hogar.', 10, 'Hasbro', 359.0E0);
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'Nombre', N'Descripcion', N'RestriccionEdad', N'Compania', N'Precio') AND [object_id] = OBJECT_ID(N'[Toys]'))
...
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'Nombre', N'Descripcion', N'RestriccionEdad', N'Compania', N'Precio') AND [object_id] = OBJECT_ID(N'[Toys]'))
    SET IDENTITY_INSERT [Toys] ON;
INSERT INTO [Toys] ([Id], [Nombre], [Descripcion], [RestriccionEdad], [Compania], [Precio])
VALUES (52, 'Laptop Aprende Conmigo', '¡Esta laptop de juguete mantiene a los bebés ocupados!', 1, 'Fisher Price', 539.0E0);
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'Nombre', N'Descripcion', N'RestriccionEdad', N'Compania', N'Precio') AND [object_id] = OBJECT_ID(N'[Toys]'))
    SET IDENTITY_INSERT [Toys] OFF;
GO
...
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'Nombre', N'Descripcion', N'RestriccionEdad', N'Compania', N'Precio') AND [object_id] = OBJECT_ID(N'[Toys]'))
    SET IDENTITY_INSERT [Toys] ON;
INSERT INTO [Toys] ([Id], [Nombre], [Descripcion], [RestriccionEdad], [Compania], [Precio])
VALUES (53, 'Woody - Toy Story 4', 'Posee un diseño lleno de detalles únicos que lo hacen una pieza perfecta', 3, 'Disney Collection', 524.29999999999995E0);
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'Nombre', N'Descripcion', N'RestriccionEdad', N'Compania', N'Precio') AND [object_id] = OBJECT_ID(N'[Toys]'))
    SET IDENTITY_INSERT [Toys] OFF;
GO
